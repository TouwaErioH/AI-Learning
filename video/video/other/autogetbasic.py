...
def judgecover(image): #判断是否有覆盖。判断方法为截取高度在70-1080，x坐标在218-219的矩形部分，逐行判断，若该行白色像素点
    image = cv2.cvtColor(image,cv2.COLOR_BGR2GRAY) #数量>8则认为该行是白色的。取70开始是为了避免坐标轴上00,10,20会将上方的空白分成2部分
...
def get2019(): #2019年没有Q4，特殊处理
    ...
    cv2.imwrite(str(2019) + '.jpg', frame)
...
def turn2years(index):  #上面得到的图片是按帧编号的，接下来取其中的55幅，代表各年。由于同一年的Q4帧间距离都比较近而不同年间距离比较远
    ...
    #savepath='./judged/'   #取前后两个index差距大于20 (即帧差距大于100,帧率30000/1001，时间差约3.44秒)
    last=-1
    temp=1965
